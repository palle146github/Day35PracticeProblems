...
public class EmployeePayRollService {
    ...
    public boolean addEMployeePayRollToDataBase() throws Exception {
        try {
            Connection connection = driver();
            PreparedStatement statement = connection.prepareStatement("insert into employee_payroll(name,gender,salary,start,phonenumber,address,department,Basic_Pay,Deductions,Taxable_Pay,Income_Tax,Net_Pay) values('Rahiman','M',200000,'2010-12-06','7598130306','Kurnool','HR',25000.0,24650.00,43500.00,65346.00,76456.00);");
            boolean response = statement.execute();
            if (response) {
                ResultSet resultSet = statement.getResultSet();
                while (resultSet.next())
